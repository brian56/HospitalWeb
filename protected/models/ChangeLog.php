...
class ChangeLog extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('hospital_id, user_id, table_name, change_type, description, date_create', 'required'),
			array('hospital_id, user_id', 'numerical', 'integerOnly'=>true),
			array('change_type', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, hospital_id, user_id, table_name, change_type, description, date_create', 'safe', 'on'=>'search'),
		);
	}
